...
def evaluate_classifier(args,train_loader, test_loader, device, epoch):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for data, preds in test_loader:
                ...
                output = model_18(data)
                test_loss += F.nll_loss(output, preds, reduction='sum').item() # sum up batch loss
                pred_index = output.argmax(dim=1, keepdim=True) # get the index of the max log-probability
                correct += pred_index.eq(preds.view_as(pred_index)).sum().item()

        test_loss /= len(test_loader.dataset)

        print('\nResnet18 Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
    ...
    yield 'ResNet18 (F1 score={:.3f})'.format(f1_score), precision, recall, thresholds
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('\nResnet34 Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            test_loss, correct, len(test_loader.dataset),
            100. * correct / len(test_loader.dataset)))
